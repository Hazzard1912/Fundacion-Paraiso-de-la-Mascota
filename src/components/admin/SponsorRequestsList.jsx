...
/**
 * Componente para mostrar y gestionar la lista de solicitudes de apadrinamiento
 * 
 * @param {Object} props
 * @param {Array} props.initialRequests - Datos iniciales de solicitudes de apadrinamiento
 */
const SponsorRequestsList = ({ initialRequests = [] }) => {
    ...
    useEffect(() => {
        ...
        if (filters.search) {
            ...
            result = result.filter(request =>
                ...
                (request.sponsorName && request.sponsorName.toLowerCase().includes(searchLower)) ||
                (request.sponsorEmail && request.sponsorEmail.toLowerCase().includes(searchLower)) ||
                (request.sponsorPhone && request.sponsorPhone.toLowerCase().includes(searchLower))
            );
        }
    ...
    const handleApprove = async (requestId) => {
        ...
        try {
            const response = await fetch('/api/admin/update-sponsor-request', {
                method: 'POST',
                headers: {
            ...
            <div className="bg-white shadow rounded-lg overflow-hidden">
                ...
                {filteredRequests.length === 0 ? (
                    <div className="p-6 text-center text-gray-500">
                        No se encontraron solicitudes de apadrinamiento.
                    </div>
                ) : (
                                    ...
                                    <tr key={request.requestId}>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            {request.requestStatus === "pending" ? (
                                                ...
                                                <a href={`/admin/apadrinamientos/${request.requestId}`} className="text-blue-600 hover:text-blue-900" title="Detalles">
                                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
